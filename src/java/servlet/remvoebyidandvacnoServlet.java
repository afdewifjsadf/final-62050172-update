 ...
 * @author tana_
 */
public class remvoebyidandvacnoServlet extends HttpServlet {
        ...
        if (listId.contains(id)) {
            ...
            synchronized (getServletContext()) {
                ...
                for (Studentvaccinedata x : stdList) {
                    ...
                    if (x.getVacno() > Integer.parseInt(no)) {
                        x.setVacno(x.getVacno() - 1);
                        StudentVaccineDataTable.updateStudent(x);
                    }
                }
                StudentVaccineDataTable.removeStdent(std);
